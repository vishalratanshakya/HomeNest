    ...
    const unsubscribe = bookingService.getOwnerBookings(
      ...
      (data) => {
        const filteredData = (data || [])
          .filter(b => !b.type || b.type === 'booking')
          .map(b => ({
            id: b.id,
            propertyId: b.propertyId || b.id,
            customer: b.clientName || b.userName || 'Guest User',
            email: b.userEmail || 'No email provided',
            phone: b.contactNumber || b.userPhone || 'No phone provided',
            propertyName: b.propertyName || 'Property Visit',
    ...
    <div className="flex min-h-screen bg-gray-100 font-sans">
      ...
      <main className="flex-1 lg:ml-72 ml-0 min-h-screen bg-slate-50/50 transition-all duration-300">
        ...
        <div className="px-4 sm:px-10 py-6 sm:py-10">
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 tracking-tight">Property Visit Bookings</h1>
            <p className="text-xs sm:text-sm text-gray-500 mt-1 font-medium">Manage visit dates, schedules, and confirmations with potential clients</p>
          </div>
          ...
          <div className="space-y-6">
                ...
                {filteredItems.map((item, idx) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-[200px]">
                      <h4 className="text-base font-bold text-gray-900">{item.propertyName}</h4>
                    ...
                    <div className="min-w-[180px]">
                      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1 flex items-center gap-1">
                        <Calendar className="w-3 h-3 text-indigo-500" /> Visit Date & Time
                      </p>
                      <p className="text-xs font-black text-gray-900">{item.visitDate}</p>
                      <p className="text-[11px] font-bold text-slate-500 mt-0.5">{item.visitTime}</p>
                    </div>
            ...
            {!loading && filteredItems.length === 0 && (
              <div className="py-24 text-center">
                ...
                <p className="text-gray-500 mt-2">No active property visit booking records found.</p>
              </div>
            )}
          </div>
        </div>
