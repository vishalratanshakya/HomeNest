...
export default function RentPage() {
  ...
  const filtered = properties.filter(item => {
    ...
    if (priceRange === 'below-10k') priceMatch = price < 10000;
    else if (priceRange === '10k-20k') priceMatch = price >= 10000 && price <= 20000;
    else if (priceRange === '20k-30k') priceMatch = price >= 20000 && price <= 30000;
    else if (priceRange === 'above-30k') priceMatch = price > 30000;
        ...
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            <aside className={`fixed inset-y-0 left-0 w-80 bg-white z-[70] p-6 shadow-2xl transition-transform duration-300 lg:static lg:w-[320px] lg:z-0 lg:p-0 lg:bg-transparent lg:shadow-none lg:translate-x-0 ${showMobileFilters ? 'translate-x-0' : '-translate-x-full'}`}>
              <div className="bg-white rounded-3xl p-6 lg:border lg:border-gray-100 lg:shadow-sm lg:sticky lg:top-24 h-full lg:h-fit overflow-y-auto lg:overflow-visible">
                ...
                <div className="space-y-6 pb-10 lg:pb-0">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full bg-gray-50 border-none rounded-xl pl-10 pr-4 py-3 text-sm font-medium focus:ring-2 focus:ring-emerald-500/20"
                        value={searchName}
                  ...
                  {/* Price Range */}
                  <div>
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 block">Price Range</label>
                    <select
                      className="w-full bg-gray-50 border-none rounded-xl px-4 py-3 text-sm font-bold text-gray-600 focus:ring-2 focus:ring-emerald-500/20 cursor-pointer"
                      value={priceRange}
                  ...
                  <button 
                    onClick={() => setShowMobileFilters(false)}
                    className="w-full lg:hidden py-4 bg-emerald-600 text-white rounded-2xl font-black uppercase tracking-widest shadow-lg shadow-emerald-100 active:scale-95 transition-all mt-4"
                  >
                    Apply Filters
                  </button>
                </div>
          ...
          <main className="flex-1">
            <div className="mb-6 flex items-center justify-between bg-white p-5 sm:p-6 rounded-3xl shadow-sm border border-gray-100">
              <div>
                <h1 className="text-xl sm:text-2xl font-black text-gray-900 uppercase tracking-wider">Rent Properties</h1>
            ...
            {!loading && (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {filtered.map((item, idx) => {
                  ...
                  const rent = Number(item.price || item.pricing?.price || 0);
                  ...
                  return (
                    <div
                      ...
                      onClick={() => navigate(`/user/rent/${item.id}`)}
                    >
                      <div className="relative h-52 overflow-hidden">
                        ...
                        />
                        <span className="absolute top-3 left-3 px-2.5 py-1 bg-emerald-600 text-white rounded-lg text-[9px] font-black uppercase tracking-widest shadow-md">For Rent</span>
                        <button 
                        ...
                        <div className="flex items-center justify-between gap-2 pt-2 border-t border-gray-50">
                          <span className="text-base font-bold text-primary-600 min-w-0 flex-1 truncate">
                            {(() => {
                              ...
                              if (!p) return 'Contact';
                              if (p >= 10000000) return `₹${(p / 10000000).toFixed(1)}Cr /mo`;
                              if (p >= 100000) return `₹${(p / 100000).toFixed(1)}L /mo`;
                              return `₹${(p / 1000).toFixed(0)}k /mo`;
                            })()}
                          </span>
