...
const types = ['Apartment', 'Villa', 'Commercial', 'Land', 'Studio'];
const baseImages = [
    ...
    for (const listingType of ['sell', 'rent']) {
      for (const pType of types) {
        for (let i = 0; i < 10; i++) {
          ...
          let price = listingType === 'sell' 
            ? Math.floor(Math.random() * 50000000) + 5000000 
            : Math.floor(Math.random() * 150000) + 15000;
          
          if (pType === 'Land') {
             price = listingType === 'sell' ? Math.floor(Math.random() * 10000000) + 1000000 : Math.floor(Math.random() * 50000) + 10000;
          }

          const p = {
            title: `Premium ${pType} in Prime Location ${i+1}`,
            propertyType: pType,
            type: pType,
            category: pType.toLowerCase(),
            listingType: listingType,
            furnishingStatus: pType === 'Land' ? 'Unfurnished' : (i % 2 === 0 ? "Fully Furnished" : "Semi-Furnished"),
            shortDescription: `A fantastic ${pType.toLowerCase()} available for ${listingType}.`,
            fullDescription: `This is a brilliantly designed ${pType.toLowerCase()} perfect for families or businesses looking for high-quality spaces. Features excellent connectivity, great views, and prime placement.`,
            bhkType: pType === 'Land' || pType === 'Commercial' ? 'N/A' : `${(i % 4) + 1} BHK`,
            bedrooms: pType === 'Land' || pType === 'Commercial' ? 0 : (i % 4) + 1,
            bathrooms: pType === 'Land' ? 0 : (i % 3) + 1,
            price: price,
            pricing: {
              price: price,
              negotiable: true
          ...
          count++;
          console.log(`Added ${listingType.toUpperCase()} ${pType} - ${count}/100`);
        }
      }
    }
    console.log("Seeding completely finished! Added 100 properties.");
    process.exit(0);
  } catch(e) {
